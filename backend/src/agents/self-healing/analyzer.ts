...
import { TestFailure, AnalysisResult } from './index';
import { FailurePattern } from './detector';

export class FailureAnalyzer {
  /**
   * Analyze failure details using LLM (Groq)
   */
  async analyzeFailure(failure: TestFailure, pattern: FailurePattern): Promise<AnalysisResult> {
   ...
   */
  private extractElementsFromError(error: string): string[] {
   ...
   */
  private generateReasons(failureType: string, failure: TestFailure): string[] {
    ...
    switch (failureType) {
      case 'selector_not_found':
        reasons.push(
          ...
          'Element is hidden or behind overlay',
          'Selector uses outdated locator',
          'Element rendered dynamically'
        );
        break;
      case 'timeout':
        reasons.push(
          'Page is slow to load',
          'Server taking longer to respond',
          'Network latency',
          'Wait timeout is too short'
        );
        break;
      case 'stale_element':
        reasons.push(
          ...
          'DOM re-rendered',
          'Element was replaced in DOM tree',
   ...
   */
  private calculateSeverity(failureType: string): 'critical' | 'high' | 'medium' | 'low' {
